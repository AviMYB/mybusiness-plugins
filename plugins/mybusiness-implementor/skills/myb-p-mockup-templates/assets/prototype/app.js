...
import { nativeWorkflows } from './native-workflows.js';
...
// Add new demo collections without discarding the reviewer's existing local edits.
const seed=makeSeed();
for(const key of ['activityRecords','reports'])if(!Array.isArray(state[key]))state[key]=seed[key];
const freshUI = () => ({ query:'', status:'', sort:1, page:1, size:10, owner:'',email:'',createdFrom:'',createdTo:'',settingsSection:'',dashboardTab:'', error:false, dashboardOwner:'', week:0, wizardStep:0, wizardDone:false, wizard:{title:'',email:'',owner:'צוות 1',date:'2026-09-23'}, document:structuredClone(state.document), inboxQuery:'', conversation:'conversation-1', drafts:{}, reportId:state.reports.find(r=>r.default)?.id||'',reportFilters:{},reportsPage:1,reportsSize:10,reportMenu:false,importValidated:false, activityFilters:{},activityPage:1,activitySize:10,knowledgeQuery:'',knowledgeCategory:'' });
let ui = freshUI();
ui.reportId=state.reports.find(r=>r.default)?.id||'';
let page = '';
let toastTimer;
let lastFocus;
...
function render() {
  ...
  document.querySelector('#page-actions').innerHTML=page==='records'?'<button class="outline small" data-action="new-record">＋ לקוח חדש</button>':page==='kanban'?'<a class="outline small" href="#records">טבלה</a><button class="outline small" data-action="new-record">＋ מכירה חדשה</button>':page==='activity'?'<button class="outline small" data-action="new-activity">＋ פעילות חדשה</button>':'';
  document.title=(nativeTitles[page]||'ספריית התבניות')+' · MyBusiness · הדגמה';
  main.innerHTML=views[page]({state,ui});
  const navItems=[['dashboard','מבט-על','chart'],['records','לקוחות','building'],['kanban','מכירות','board'],['calendar','יומן','calendar'],['activity','פעילויות','calendar'],['reports','דוחות','document'],['settings','הגדרות','settings']];
...
function openDialog(title,body,sheet=false) {
  dialog.classList.remove('activity-sheet','report-editor');
...
dialog.addEventListener('keydown',e=>{
  if(e.key!=='Tab')return;
  const items=Array.from(dialog.querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),a[href]')).filter(el=>el.getClientRects().length);
...
const native=nativeWorkflows({getState:()=>state,getUI:()=>ui,render,openDialog,closeDialog,save,toast});
Object.assign(actions,native.actions);
...
document.addEventListener('submit',e=>{
  ...
  if(['record','event','wizard','message','document','activity-record'].includes(kind)){
  ...
  if(native.submit(kind,form,data,e))return;
  if(kind==='record'){
